...
function formatDurationHms(seconds) {
  const total = Math.round(parseFloat(seconds) || 0);
...
function streamPdf(res, { rows, totalSeconds, totalAmount, start, end, workspaceName }) {
  ...
  doc.font('Helvetica-Bold').fontSize(12).fillColor('#111111')
    .text(`Total: ${formatDurationHms(totalSeconds)} (${(totalSeconds / 3600).toFixed(2)} h)`);
  ...
  if (rows.length === 0) {
    ...
    rows.forEach((r, idx) => {
      ...
      const hours = formatDurationHms(r.duration_seconds);
    ...
    doc.font('Helvetica-Bold').fontSize(10).fillColor('#111111')
      .text(`TOTAL: ${formatDurationHms(totalSeconds)} (${(totalSeconds / 3600).toFixed(2)} h)  |  R$ ${totalAmount.toFixed(2).replace('.', ',')}`, 40);
  }
